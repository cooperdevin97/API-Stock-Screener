...
my_dict = Counter(my_str.lower())
print(my_dict)  # 👉️ Counter({'b': 3, 'o': 1, 'y': 1})

print(my_dict['b'])  # 👉️ 3
print(my_dict['o'])  # 👉️ 1


tickers = si.tickers_sp500(2022)

start = dt.datetime.now() - dt.timedate(days=365)
end = dt.datetime.now()
...
return_list = []
final_df = pd.DataFrame(columns=['Ticker', 'Latest_Price', 'Score', 'PE_Ratio',
                        'PEG_Ratio', 'SMA_150', 'SMA_200', '52_Week_Low', '52_Week_High'])
...
best_performers = pd.DataFrame(list(zip(ticker, return_list)), columns=[
                               'Ticker ', 'Returns Compared'])
best_performers['Score'] = best_performers['Returns Compared'].rank(
    pct=True) * 100
best_performers = best_performers[best_performers['Score']
                                  >= best_performers['Score'].quantile(0, 8)]

for ticker in best_performers['Ticker']:
    try:
        df = pd.read_csv(f'stock_data/{ticker}.csv', indeex_col=0)
        moving_averages = [150, 200]
        for ma in moving_averages:
            df['SMA_' +
                str(ma)] = round(df['Adj Close'].rolling(window=ma).mean(), 2)
        ...
        low_52week = round(min['Low'][-(52 * 5):], 2)
        high_52week = round(min['High'][-(52 * 5):], 2)
        score = round(
        ...
        if condition_1 and condition_3 and condition_2 and condition_4 and condition_5:
